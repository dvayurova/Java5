...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public Optional<Message> findById(Long id) throws SQLException {
        ...
        if (resultSet.next()) {
            ...
            if(resultSet.getString("date") != null){
                ...
                dateTime = LocalDateTime.parse(resultSet.getString("date"), formatter);
            }
    ...
    public void update(Message message) throws SQLException {
        Connection connection = dataSource.getConnection();
        PreparedStatement statement = connection.prepareStatement("update chat.messages set author = ?, room = ?, message = ?, date = ? where id = ?", Statement.RETURN_GENERATED_KEYS);
        statement.setLong(1, message.getAuthor().getId());
        statement.setLong(2, message.getRoom().getId());
        statement.setString(3, message.getText());
